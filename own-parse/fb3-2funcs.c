...
struct ast *
newexplist(int nodetype, struct ast *l, struct ast *r, char brace)
{
  struct explist *a = malloc(sizeof(struct explist));
...
newchr(char d)
{
  struct chr *a = malloc(sizeof(struct chr));
...
eval(struct ast *a)
{
  ...
  switch(a->nodetype) {
    /* constant */
  case 'K': v = ((struct numval *)a)->number; printf("%f",v); break;
 // case '{': printf("{"); eval(a->l); break; case '(': printf("("); eval(a->l);	 break;
  ...
  case '=': v = ((struct symasgn *)a)->s->value =
    ...
    /* expressions */
  case '+': /*v =*/ eval(a->l); printf("+"); eval(a->r); break;
  case '-': /*v =*/ eval(a->l); printf("-"); eval(a->r); break;
  case '*': /*v =*/ eval(a->l); printf("*"); eval(a->r); break;
  ...
  case 'L': 
            eval(a->l); printf(","); eval(a->r); break;

  case 'F': callbuiltin((struct fncall *)a); break;
...
yyerror(char *s, ...)
{
  ...
  fprintf(stderr, "Неверный формат выражения, строка - %d: ошибка: ", yylineno-1);
